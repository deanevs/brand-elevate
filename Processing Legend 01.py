...
def format_category(lst=[]):
    in_list = str(lst)
    char_remove = str.maketrans("", "", "['][\"]{}")
    tmp = in_list.translate(char_remove)
    if '|' in tmp:
        tmp = tmp.replace('|', ',')
    if '| ' in tmp:
        tmp = tmp.replace('| ',',')
...
# used for alternate image only, changing | to ,
def fmt_img_sep(img=str):
    if '|' in img:
        return img.replace('|',',')

# used for creating unique product names for url.  However, does not
# allow for 2 colours having same code
...
def check_url(url = str, url_list = []):
    s = url
    while [s.upper()] in url_list:
        print('Before s =' + s)
        tmp = random_permutation('ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890', 2)
        s = s + '-' + ''.join(tmp)
        print(s)
    url_list.append([s.upper()])
    return s
    ...
    with open(outfile,'w',encoding='utf8',newline='') as fout:
        ...
        for row in dr:
            ...
            *************DO ADVANCED PRICING *****************
            ...
            """
            ********DO DECORATION OPTIONS********
            """
            dec_option_return = []
            if decoration_options_available:
                tmp = []
                tmp = str(decoration_options_available).split('|')
                length = len(tmp)
                for n in range(0,length):
                    if tmp[n] in legend.LEG_DECORATIONS:
                        opt = legend.LEG_DECORATIONS[tmp[n]]
                        if not opt in dec_option_return:
                            dec_option_return.append(opt)
                    else:
                        print (tmp[n] + ' - NOT IN LEG_DECORATIONS')
            ...
            **********DO UNIQUE NAME******
            ...
            while [s.upper()] in name_list:
                tmp = random_permutation('ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890',2)
                s = s + ' ' + ''.join(tmp)
                #print (s)

            name_list.append([s.upper()])
            product_name = s
            ...
            ********DO CATEGORIES**********
            ...
            category_return = format_category((temp_list))
            ...
            ***********DO URL-KEY *****************
            ...
            # url_key_return = str
            # tmp_url = str
            # tmp_url = url_key_format(product_name)
            # while not tmp_url in url_list:
            #     tmp_url = url_key_format(tmp_url)
            # url_list.append(tmp_url)
            #
            # url_key_return = tmp_url
            ...
            ***********DO CONFIGURABLE*************
            ...
            #set('decoration_type' ,fmt(dec_option_return))
            #set('decoration_options_available',format_category(dec_option_return))
            ...
            df_headers = [
                'sku', 'product_type', 'name', 'visibility', 'url_key', 'base_image',
                'small_image', 'thumbnail_image', 'swatch_image', 'color', 'additional_images',
            ...
            if num_colours < 2:   # SIMPLE
                ...
                set('url_key', url_key_format(check_url(product_name,url_list)))
                ...
                set('additional_images',fmt_img_sep(update_path(additional_images)))
                ...
                if tier_price == True:
                    ...
                    for n in range(0,num_tiers):
                        set2('tier_price_qty',format_price(str(qty_arr[n])))
                        set2('tier_price',format_price(str(price_arr[n])))
            ...
            # CONFIGURABLE TYPE
                ...
                df.set_value(0, 'name', product_name)
                df.set_value(0, 'visibility', 'Catalog, Search')
                df.set_value(0, 'url_key', url_key_format(check_url(product_name,url_list)))
                df.set_value(0, 'base_image', update_path(base_image,'base'))
                df.set_value(0, 'small_image', update_path(base_image,'small'))
                df.set_value(0, 'thumbnail_image',update_path(base_image,'thumb'))
                df.set_value(0, 'swatch_image', '')
                df.set_value(0, 'color', '')
                df.set_value(0, 'additional_images', fmt_img_sep(update_path(additional_images)))
                df.set_value(0, 'configurable_variations', '')

                for row in range(1, num_ext_rows + 1):  # plus 1 since the base plus extra
                    tmp_sku = check_url(str(product_code) + '-' + str(two_code_list[cnt]),url_list)

                    df.set_value(row, 'sku', tmp_sku)
                    df.set_value(row, 'product_type', 'simple')
                    df.set_value(row, 'name', str(tmp_sku).lower())
                    df.set_value(row, 'visibility', 'Not Visible Individually')
                    df.set_value(row, 'url_key', url_key_format(check_url(str(tmp_sku).lower(),url_list)))
                    df.set_value(row, 'base_image',update_path(col_img_dic[colour_list[cnt]],'base'))
                    df.set_value(row, 'small_image',update_path(col_img_dic[colour_list[cnt]],'small'))
                    df.set_value(row, 'thumbnail_image',update_path(col_img_dic[colour_list[cnt]],'thumb'))
                    df.set_value(row, 'swatch_image',update_path(col_img_dic[colour_list[cnt]],'swatch'))
                    #df.set_value(row, 'color', constants.BE_CLUT[single_code_list[cnt]])
                    df.set_value(row, 'color', single_colour_list[cnt])
                    df.set_value(row, 'additional_images', fmt_img_sep(update_path(additional_images)))
