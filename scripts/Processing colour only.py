        ...
        for row in dr:
                ...
                if num_colours > 0:
                    for colour in tmp_col_lst: # check 2-tone
                        ...
                        if ',' in colour:
                            ...
                            single_code_list.append(prim_BE_code)   # Gray
            ...
            ***********DO CONFIGURABLE*************
            ...
            if num_colours < 2:   # SIMPLE
                ...
                set('base_image', update_path(base_image))
                set('small_image',update_path(base_image))
                set('thumbnail_image',update_path(base_image))
                set('swatch_image','')
           ...
           # configurable type
                ...
                df.set_value(0, 'base_image', update_path(base_image))  # DONE
                df.set_value(0, 'small_image', update_path(base_image))  # DONE
                df.set_value(0, 'thumbnail_image',update_path(base_image))  # DONE
                ...
                for row in range(1, num_ext_rows + 1):  # plus 1 since the base plus extra
                    ...
                    df.set_value(row, 'base_image',update_path(col_img_dic[colour_list[cnt]]))  # DONE
                    df.set_value(row, 'small_image',update_path(col_img_dic[colour_list[cnt]]))  # DONE
                    df.set_value(row, 'thumbnail_image',update_path(col_img_dic[colour_list[cnt]]))  # DONE
                    df.set_value(row, 'swatch_image',update_path(col_img_dic[colour_list[cnt]]))  # empty on first
                    df.set_value(row, 'color', constants.BE_CLUT[single_code_list[cnt]])  # DONE
                    df.set_value(row, 'color', colour_list[cnt])  # DONE
